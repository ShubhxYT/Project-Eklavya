// Vendored from wass08/wawa-lipsync v0.0.2 (MIT, github.com/wass08/wawa-lipsync).
// Changes from upstream:
// - added connectStream(): tap a WebRTC MediaStream into the analyser
//   (upstream only accepts HTMLMediaElement with .src, which WebRTC tracks
//   never have)
// - the analyser is tap-only for connectStream: it is NOT connected to
//   audioContext.destination, so playback stays on the <audio> element and
//   is not doubled.

import { average } from "./mathUtil";
...
export class Lipsync {
  ...
  // public for overlay debugging (audio context state exposure)
  ...
  // public: the avatar lerps smoother between visemes while in vowel state
  ...
  // Tap a live stream for analysis via MediaStreamSource.
  // Chrome (crbug 40094084 / 121673): remote WebRTC streams are routed
  // straight to hardware and a MediaStreamAudioSourceNode taps silence
  // UNLESS the stream is also attached to a MUTED <audio> element — muted
  // forces playout through the graph (and prevents double audio with the
  // host UI's own element). Analyser is tap-only (no destination).
  ...
  connectStream(stream: MediaStream) {
    ...
    if (!this.hiddenAudio) {
      ...
      this.hiddenAudio.autoplay = true;
      this.hiddenAudio.muted = true;
      this.hiddenAudio.style.display = "none";
      document.body.appendChild(this.hiddenAudio);
    }
